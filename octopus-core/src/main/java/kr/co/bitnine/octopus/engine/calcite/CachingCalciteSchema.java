 ...
 */
final class CachingCalciteSchema extends CalciteSchema {
    ...
    private abstract class AbstractCached<T> implements Cached<T> {
        ...
        public T get(long now) {
            ...
            if (checked == Long.MIN_VALUE
                    || getSchema().contentsHaveChangedSince(checked, now)) {
                t = build();
            }
            checked = now;
            return t;
     ...
     */
    private static final class SubSchemaCache {
        /**
         ...
         */
        private final LoadingCache<String, CalciteSchema> cache;
